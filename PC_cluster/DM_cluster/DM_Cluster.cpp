...
class DC_Cluster{
    ...
    DC_Cluster(double input_thresh, double close_defined, double ratio_thresh, double EC_dis_lower_, double EC_dis_upper_):angle_threshold(input_thresh),close_defined(close_defined),ratio_threshold(ratio_thresh),EC_dis_lower(EC_dis_lower_),EC_dis_upper(EC_dis_upper_){}
    ...
    std::array<int, 131072>  DC_cluster(py::array_t<double> input_array_x_,py::array_t<double> input_array_y_, py::array_t<double> input_array_z_, py::array_t<double> input_array,py::array_t<int> mm_seed,py::array_t<int> nn_seed, int num_seed){
...
PYBIND11_MODULE(DC_Cluster, m) {
    py::class_<DC_Cluster>(m, "DC_Cluster")
    	.def(py::init<float, float, float, float, float>())
        .def("Condition_angle", &DC_Cluster::Condition_angle)
        .def("Increase_queue", &DC_Cluster::Increase_queue)
        .def("calculate_eu_dis",&DC_Cluster::calculate_euclidean_distance)
        .def("DC_Cluster", &DC_Cluster::DC_cluster);
}
